...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!otpResult.success) {
      // Log the OTP failure but don't delete the user
      await db.adminAction.create({
        data: {
          id: dropid("act"),
          adminId: result.id,
          actionType:'SYSTEM_CONFIG',
          description: `OTP sending failed: ${otpResult.error}`,
          ipAddress: req.headers.get('x-forwarded-for') || 'unknown'
        }
      });
      ...
      return NextResponse.json(
        {
          success: false,
          error: "Account created but verification email failed. Please try resending.",
          data: {
            userId: result.id,
            email: result.email,
          },
    ...
    const { passwordHash, ...adminWithoutSensitive } = result;
